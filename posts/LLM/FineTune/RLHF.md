...
#### Stage 1: Supervised Fine-Tuning (SFT)
  ...
  * **Backpropagation Mechanism:** This is standard supervised learning.
    ...
    3.  The cross-entropy loss is calculated between the model's predicted probability distribution and the actual next token $y\_i$.
...
This is the most complex stage. We are now fine-tuning our SFT model (the policy) using the frozen Reward Model as the source of feedback.
  ...
  * **Objective and Mathematics (PPO):**
    ...
    $$$$  \* $\\epsilon$ is a small hyperparameter (e.g., 0.2).
      * `clip(...)` constrains the ratio to be within the range $[1-\\epsilon, 1+\\epsilon]$.
...
3.  **Output:** An SFT model, $\\pi^{SFT}$.

**Stage 2: Reward Model (RM) Training**

1.  **Data Collection:** For a set of prompts, generate multiple responses using the $\\pi^{SFT}$ model.
...
4.  **Training:** Initialize a new model with the weights of $\\pi^{SFT}$, replace the language modeling head with a scalar prediction head, and train it on the pairwise comparison dataset.
5.  **Output:** A Reward Model, $\\pi^{RM}$.

**Stage 3: Reinforcement Learning (RL) via PPO**

1.  **Initialization:**
      * The **Actor** is initialized with the weights of $\\pi^{SFT}$.
      * The **Critic** is also initialized with weights from $\\pi^{SFT}$ (with a value head).
      * Load the frozen $\\pi^{RM}$ and a frozen copy of $\\pi^{SFT}$ (as the reference model).
2.  **The RL Loop (iterate for many episodes):**
    a. **Sample Prompts:** Get a batch of prompts from a dataset.
    b. **Generate Responses (Rollout):** The Actor $\\pi^{RL}$ generates a response for each prompt. Store the log-probabilities of the generated tokens.
    c. **Calculate Rewards:** For each `(prompt, response)` pair:
    i.  The $\\pi^{RM}$ calculates the reward score.
...
# A simple function to format the dataset for the trainer
def formatting_func(examples):
    # The RewardTrainer expects columns 'chosen' and 'rejected'
    # We need to tokenize them beforehand
    kwargs = {"padding": "max_length", "truncation": True, "max_length": 512, "return_tensors": "pt"}
    chosen_tokens = rm_tokenizer(examples['chosen'], **kwargs)
    rejected_tokens = rm_tokenizer(examples['rejected'], **kwargs)
    return {
...
# We need to manually tokenize the dataset this way for RewardTrainer
# A simplified map is shown here; a more robust implementation is needed
# pref_dataset_formatted = pref_dataset.map(formatting_func)
...
# Note: The RewardTrainer expects pre-tokenized 'chosen' and 'rejected' fields.
# The code below is a conceptual representation.
# reward_trainer = RewardTrainer(
#     model=rm_model,
#     tokenizer=rm_tokenizer,
#     args=training_args,
#     train_dataset=pref_dataset_formatted,
# )
...
# 4. The RL Training Loop (Conceptual)
...
#         # This is handled internally in ppo_trainer.step, which uses a reward model or function
#         # For demonstration, let's assume a reward function exists
#         rewards = [torch.tensor(1.0) for _ in batch['response']] # Placeholder
#
