...
3.  **Use the Log-Likelihood:** Products are mathematically cumbersome to differentiate. Since the logarithm is a monotonically increasing function, maximizing a function is the same as maximizing its logarithm. This turns our product into a much friendlier sum.
    ...
    $$
    \hat{\theta}_{MLE} = \arg\max_{\theta} \log \left( \prod_{i=1}^{N} P(x_i | \theta) \right) = \arg\max_{\theta} \sum_{i=1}^{N} \log P(x_i | \theta)
    $$
...
* **Negative Log-Likelihood:** Let's find the total negative log-likelihood for our dataset $D = \{(x_1, y_1), ..., (x_N, y_N)\}$.
    ...
    $$
    \begin{aligned}
    -\log L(\theta|D) &=& -\log \prod_{i=1}^{N} P(y_i | x_i; \theta) = -\sum_{i=1}^{N} \log P(y_i | x_i; \theta)
...
* **Likelihood:** Let the true label $y_i$ be a one-hot encoded vector (e.g., `[0, 0, 1, 0]` for class 3). The probability of observing this true label is simply the probability our model assigned to that class.
    ...
    $$
    P(y_i | x_i; \theta) = \prod_{c=1}^{C} p_{i,c}^{y_{i,c}}
    $$
...
* **Negative Log-Likelihood:**
    $$
    -\log L(\theta|D) = -\sum_{i=1}^{N} \log P(y_i | x_i; \theta)
    $$
...
* **Log-Prior:** The PDF for this prior is $P(\theta) \propto \exp\left(-\frac{\|\theta\|^2_2}{2\beta^2}\right)$, where $\|\theta\|^2_2 = \sum_j \theta_j^2$ is the squared L2-norm. The log of the prior is:

    $$
    \log P(\theta) = -\frac{\|\theta\|^2_2}{2\beta^2} + \text{constant}
    $$
...
* **The Loss Function:** To get our final MAP objective, we minimize the negative log-posterior.
    ...
    $$
  \begin{aligned}
    \text{Loss}(\theta) &=& -\log P(D | \theta) - \log P(\theta)
    &\propto& \left[ \sum_{i=1}^{N} (y_i - f(x_i; \theta))^2 \right] + \left[ \frac{\sigma^2}{\beta^2} \|\theta\|^2_2 \right]
  \end{aligned}
    $$
    ...
    $$
    \text{Loss}(\theta) = \underbrace{\sum_{i=1}^{N} (y_i - f(x_i; \theta))^2}_{\text{MSE (from Likelihood)}} + \underbrace{\lambda \|\theta\|^2_2}_{\text{L2 Regularization (from Prior)}}
    $$
